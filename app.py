...
with st.sidebar:
    ...
    selected = option_menu(menu_title="Navigation",options=["Dashboard","Dataset","Statistics","Visualization",
...
if selected=="Dashboard":
    ...
    with right:
        st.subheader("DataSet Informtion")
        st.write("Row :",len(df))
...
#prediction
    ...
    if st.button("Prediction user category",use_container_width=True):
        ...
        with col1:
            st.subheader("Prediction Summerry")
            if cluster == 0:
                st.info("""
                Reason
                - Low Screen Time
